...
@Singleton
class DataGenerator @Inject() (auditEvents: AuditEvents, applicationConfig: ApplicationConfig)(implicit
  val ec: ExecutionContext
) extends DataParser with Metrics with Logging {
  ...
  def getErrors(iterator: Iterator[String])(implicit
    schemeInfo: SchemeInfo,
    hc: HeaderCarrier,
    request: Request[_]
  ): Either[ErsError, ListBuffer[SchemeData]] = {
    var rowNum                             = 0
    ...
    Try {
            ...
            (1 to rd._2).foreach { _ =>
              rowNum match {
                case count if count < 9 =>
                  logger.debug(
                    "[DataGenerator][getErrors] GetData: incRowNum if count < 9: " + count + " RowNum: " + rowNum
                  )
                  incRowNum()
                case 9                  =>
                  logger.debug(
                    "[DataGenerator][getErrors] GetData: incRowNum if  9: " + rowNum + "sheetColSize: " + sheetColSize
                  )
      ...
      if (schemeData.foldLeft(0)((sum, obj) => sum + obj.data.size) == 0) {
        return Left(
          NoDataError(s"${ErrorResponseMessages.dataParserNoData}", s"${ErrorResponseMessages.dataParserNoData}")
        )
      }
      ...
      case Failure(e)      =>
        ...
        Left(ERSFileProcessingException("System error during file processing", s"Unexpected error: ${e.getMessage}"))
    }
  }

  def getValidator(
    sheetName: String
  )(implicit schemeInfo: SchemeInfo, hc: HeaderCarrier, request: Request[_]): Either[ErsError, DataValidator] =
    ersSheetsConf(schemeInfo).flatMap { sheetsConf =>
      try
        ...
        case _: ConfigException.Missing =>
          ...
          auditEvents.fileProcessingErrorAudit(schemeInfo, sheetName, errorMsg)
          logger.error(
            s"[getValidator] $errorMsg for sheet name: $sheetName and scheme type: ${schemeInfo.schemeType}."
          )
          Left(ErsSystemError(errorMsg, "Config missing"))
      }
    }

  def getValidatorAndSheetInfo(sheetName: String, schemeInfo: SchemeInfo)(implicit
    hc: HeaderCarrier,
    request: Request[_]
  ): Either[ErsError, (DataValidator, SheetInfo)] =
    ersSheetsConf(schemeInfo).flatMap { sheetsConf =>
      sheetsConf.get(sheetName) match {
        case Some(sheetInfo) =>
          Try(ERSValidationConfigs.getValidator(sheetInfo.configFileName)) match {
            ...
            case Failure(e)         =>
              ...
              logger.error(
                "setValidator has thrown an exception, SheetName: " + sheetName + " Exception message: " + e.getMessage
              )
              Left(ErsSystemError(s"${ErrorResponseMessages.dataParserConfigFailure}", "Could not set the validator"))
          }
        case None            =>
          auditEvents.fileProcessingErrorAudit(schemeInfo, sheetName, "Could not set the validator")
  ...
  def identifyAndDefineSheet(
    data: String
  )(implicit schemeInfo: SchemeInfo, hc: HeaderCarrier, request: Request[_]): Either[ErsError, String] =
    getSheet(data).flatMap { res =>
      ...
      if (requestSchemeType.toLowerCase == schemeInfoSchemeType.toLowerCase) {
        ...
        auditEvents.fileProcessingErrorAudit(
          schemeInfo,
          data,
          s"${res.schemeType.toLowerCase} is not equal to ${schemeInfo.schemeType.toLowerCase}"
        )
        ...
        Left(
          SchemeTypeMismatchError(
            ...
            context =
              s"${ErrorResponseMessages.dataParserIncorrectSchemeType(Some(schemeInfoSchemeType), Some(requestSchemeType))}",
  ...
  def getSheet(
    sheetName: String
  )(implicit schemeInfo: SchemeInfo, hc: HeaderCarrier, request: Request[_]): Either[ErsError, SheetInfo] =
    ersSheetsConf(schemeInfo).flatMap { sheetsConf =>
      sheetsConf.get(sheetName) match {
  ...
  def validateHeaderRow(rowData: Seq[String], sheetName: String)(implicit
    schemeInfo: SchemeInfo,
    hc: HeaderCarrier,
    request: Request[_]
  ): Either[ErsError, Int] = {
  ...
  def generateRowData(rowData: Seq[String], rowCount: Int, validator: DataValidator)(implicit
    schemeInfo: SchemeInfo,
    sheetName: String,
    hc: HeaderCarrier,
    request: Request[_]
  ): Either[ErsError, Seq[String]] = {
    ...
    Try {
      ...
      case Success(err: Option[List[ValidationError]]) =>
        ...
        err match {
          case Some(errors) if errors.nonEmpty =>
            val errorDetails = errors.map { case ValidationError(cell, _, errorId, errorMsg) =>
              s"column - ${cell.column}, error - $errorId : $errorMsg"
            }
            logger.warn(
              s"[DataGenerator][generateRowData] Validation errors found for ${schemeInfo.schemeRef} : ${errorDetails.mkString(" | ")}"
            )
        }
        err.map {
      ...
      case Failure(exception)                          =>
        logger.error(
          s"[DataGenerator][generateRowData] System error during row validation: ${exception.getMessage}",
          exception
        )
        Left(
          ErsSystemError(
