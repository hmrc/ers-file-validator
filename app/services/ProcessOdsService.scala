          ...
          case Left(ersError) =>
            ersError match {
              case userError: UserValidationError =>
                logger.warn(
                  s"[ProcessOdsService][processFile] User validation error: ${userError.message}, context: ${userError.context}, schemeRef: ${schemeInfo.schemeRef}"
                )
                deliverBESMetrics(startTime)
                Future.successful(Left(userError))
              case systemError: SystemError       =>
                logger.error(
                  s"[ProcessOdsService][processFile] System error: ${systemError.message}, context: ${systemError.context}, schemeRef: ${schemeInfo.schemeRef}"
                )
                deliverBESMetrics(startTime)
