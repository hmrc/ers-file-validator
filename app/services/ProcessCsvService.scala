...
class ProcessCsvService @Inject() (
  ...
  def processFiles(callback: UpscanCsvFileData, source: String => Source[HttpResponse, _])(implicit
    request: Request[_],
    hc: HeaderCarrier
              ...
              .via(
                Flow.fromFunction((maybeRow: Either[Throwable, List[ByteString]]) =>
                  maybeRow.flatMap(row =>
                    processRow(row, successUpload.name, callback.schemeInfo, validator, sheetInfo)
                  )
                )
              )
    ...
    Try {
      ...
      case Failure(exception)            =>
        ...
        Left(
          RowValidationError(
            "Invalid file format",
            s"Could not validate row due to unexpected structure. Error: ${exception.getMessage}",
            None
          )
        )
      ...
      case Success(Some(errors))         =>
        ...
        logger.warn(
          s"[ProcessCsvService][processRow] Validation errors found for ${schemeInfo.schemeRef} : ${errorDetails.mkString(" | ")}"
        )
