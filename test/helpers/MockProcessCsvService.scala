...
class MockProcessCsvService(
  ...
  override def formatDataToValidate(rowData: Seq[String], sheetInfo: SheetInfo): Seq[String] =
    formatDataToValidate match {
  ...
  override def extractBodyOfRequest: Source[HttpResponse, _] => Source[Either[Throwable, List[ByteString]], _] =
    mockExtractBodyOfRequest match {
      case None           => super.extractBodyOfRequest
      case Some(returner) => returner
    }

  override def processRow(
  ...
  override def sendSchemeCsv(schemeData: SubmissionsSchemeData, empRef: String)(implicit
    hc: HeaderCarrier,
    request: Request[_]
  ): Future[Option[Throwable]] = sendSchemeCsvNew match {
