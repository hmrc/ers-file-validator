...
class ProcessCsvServiceSpec
  ...
  val sheetTest: SheetInfo =
    SheetInfo("schemeType", 1, "CSOP_OptionsGranted_V4", "sheetTitle", "configFileName", List("aHeader"))
  ...
  def testProcessCsvService: ProcessCsvService =
    new ProcessCsvService(mockAuditEvents, mockDataGenerator, mockAppConfig, mockErsFileValidatorConnector)
  ...
  "processRow" should {
    "process a row and return the user validation error it contains" in {
      val sheetTest: SheetInfo =
        SheetInfo("schemeType", 1, "sheetName", "sheetTitle", "configFileName", (1 to 5).map(_.toString).toList)

      val testService   =
        new MockProcessCsvService(mockAuditEvents, mockDataGenerator, mockAppConfig, mockErsFileValidatorConnector)(
          formatDataToValidate = Some(Seq("INVALIDROW", "With", "ManyValues"))
        )
      val dataValidator = new MockDataValidator(
        mock[Config],
        Some(List(ValidationError(Cell("A", 1, "value"), "ruleId", "errorId", "errorMessage")))
      )
      val result        = testService.processRow(
        List(ByteString("INVALIDROW,With,ManyValues")),
        "Other_Grants_V4.csv",
        schemeInfo,
        dataValidator,
        sheetTest
    ...
    "process a row and return the data if there are no errors" in {
      ...
      val result        = testService.processRow(
        List(ByteString("thisIsARow,With,ManyValues")),
  ...
  "processFiles" should {
    when(mockDataGenerator.getValidatorAndSheetInfo(any(), any[SchemeInfo])(any(), any()))
      .thenReturn(
        Right((new DataValidator(ConfigFactory.load.getConfig("ers-csop-granted-validation-config")), sheetTest))
      )
    ...
    "return a user validation error when file is empty" in {
      ...
      val callback                       =
        UpscanCsvFileData(List(UpscanCallback("CSOP_OptionsGranted_V4.csv", "no", noOfRows = Some(1))), schemeInfo)
      val data                           =
        "2015-09-23,250,123.12,12.1234,12.1234,no,yes,AB12345678,no\n2015-09-23,250,123.12,12.1234,12.1234,no,yes,AB12345678,no\n2015-09-23,250,123.12,12.1234,12.1234,no,yes,AB12345678,no"
    ...
    "return a user validation error when an error occurs during the file processing" in {
      ...
      val data     =
        "2015-09-23,250,123.12,12.1234,12.1234,no,yes,AB12345678,no\n2015-09-23,250,123.12,12.1234,12.1234,no,yes,AB12345678,no\n2015-09-23,250,123.12,12.1234,12.1234,no,yes,AB12345678,no"
      when(mockDataGenerator.getValidatorAndSheetInfo(any(), any[SchemeInfo])(any(), any())).thenReturn(
        Left(
  ...
  "formatDataToValidate" should {
    ...
    "process the rowData" in {
      ...
      when(mockDataGenerator.getValidatorAndSheetInfo(any(), any())(any(), any()))
        .thenReturn(Right(mock[DataValidator], sheetTest))
  ...
  "sendSchemeCsv New" should {
    "return a None if the submission was sent successfully" in {
      ...
      when(
        mockErsFileValidatorConnector.sendToSubmissionsNew(any[SubmissionsSchemeData], any[String])(
          any[HeaderCarrier],
          any[Request[_]]
        )
      )
    ...
    "return a throwable if the submission was not sent successfully" in {
      ...
      doNothing()
        ...
        .auditRunTimeError(any[Throwable], any[String], any[SchemeInfo], any[String])(
          any[HeaderCarrier],
          any[Request[_]]
        )
