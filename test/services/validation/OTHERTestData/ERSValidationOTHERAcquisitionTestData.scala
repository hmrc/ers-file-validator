      ...
      Some(
        List(
          ValidationErrorData(
            "error.18",
            "018",
            "PAYE reference must be a 3 digit number followed by a forward slash and up to 10 more characters."
          )
        )
      ),
      // S
      ...
      Some(
        List(
          ValidationErrorData(
            "error.27",
            "027",
            "Must be a number (whole numbers must have no more than 13 digits; decimals must have no more than 13 digits before the decimal point and no more than 4 digits after it)."
          )
        )
      ),
      // AB
