...
class OTHERAcquisitionV4ValidationTest
    extends PlaySpec with ERSValidationOTHERAcquisitionTestData with ValidationTestRunner {

  "ERS Validation tests for OTHER Acquisition" should {
    val validator = new DataValidator(ConfigFactory.load.getConfig("ers-other-acquisition-validation-config"))
    runTests(validator, getDescriptions, getTestData, getExpectedResults)
    ...
    "when Column B is answered YES, column C is a mandatory field" in {
      ...
      resOpt.get must contain(
        ValidationError(
          ...
          "mandatoryB",
          "MB",
          "Enter the scheme reference number (it should be an 8 digit number)."
        )
      )
    }
...
class OTHERRestrictedSecuritiesV4_ValidationTest
    extends PlaySpec with ERSValidationOTHERRestrictedSecuritiesTestData with ValidationTestRunner {
...
class OTHEROtherBenefitsV4ValidationTest
    extends PlaySpec with ERSValidationOTHEROtherBenefitsTestData with ValidationTestRunner {
...
class OTHERConvertibleV4ValidationTest
    extends PlaySpec with ERSValidationOTHERConvertibleTestData with ValidationTestRunner {
...
class OTHEREnhancementV4ValidationTest
    extends PlaySpec with ERSValidationOTHEREnhancementTestData with ValidationTestRunner {
